...
<template>
  ...
  <div v-else>
    ...
    <div class="bg-slate-500 grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 grid-cols-3 gap-5 p-5">
      <div v-for="episode in result.episodes.results" :key="episode.id">
        <RouterLink :to="`/episodes/${episode.id}`">
          <div
            class="hover:border-2 h-[220px] p-5 flex justify-evenly items-center rounded-xl text-slate-100 bg-slate-800 flex-wrap">
